# Generate Fig. 2 (event study) for the Ferraro & Shimshack policy brief.
#
# Reproduces the paper's Figure 2(a): eq. 2's event study of the number of
# violations, by month relative to training, on treated facilities only.
# Identification is time-based -- earlier- and later-trained cohorts serve as
# control units for one another -- and coefficients are deviations from the
# normalized event-time-0 comparison period, not a contrast against a
# never-trained group. Effect is flat with no jump at the workshop.
#
...
#
# Run (from data-viz/):  Rscript code/make-fig2-event-study.R
#   -> figures/fig2-event-study.png
...
OUT_PATH  <- "figures/fig2-event-study.png"
...
# Reference period 0 (the workshop month), matching eq. 2's normalization
# beta_0 = 0; SEs clustered at the facility level.
m <- feols(viols ~ i(e, ref = 0) + rainfall + avg_temp | permit2 + time,
...
p <- ggplot(es, aes(event, est)) +
  # before/after-training framing; the workshop is month 0, so the marker sits
  # on that dot (the normalized baseline) rather than in the gap beside it
  annotate("rect", xmin = 0, xmax = 12.5, ymin = -Inf, ymax = Inf,
           fill = BAND, alpha = 0.35) +
  annotate("text", x = -6, y = 0.92, label = "BEFORE TRAINING",
           size = 2.4, color = MUTE, fontface = "bold") +
  annotate("text", x = 6.5, y = 0.92, label = "AFTER TRAINING",
