...
\noindent \textbf{Evaluating whether a policy works is hard: there is no way to see what facilities would have done without it.} On one hand, asking whether violations fell at trained facilities after the workshops could credit the training with whatever else was going on at the same time. On another hand, comparing facilities that signed up for trainings against those that didn't raises another problem: the workshops were voluntary, so a difference between the two could reflect the \emph{motivation} of the facilities that signed up rather than anything the training did.
